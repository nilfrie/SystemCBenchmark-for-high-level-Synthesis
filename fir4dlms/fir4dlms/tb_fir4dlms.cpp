//
// Implementation of the behaviour of the testbench for fir4dlms.
//
...
void test_fir4dlms::recv()
{
	...
	while (true)
	{
		...
		outdatay = y.read();
		fprintf(out_file_y, "%i\n", outdatay);

		outdataf0 = f0.read();
		fprintf(out_file_f0, "%i\n", outdataf0);

		outdataf1 = f1.read();
		fprintf(out_file_f1, "%i\n", outdataf1);

		outdatae = e.read();
		fprintf(out_file_error, "%i\n", outdatae);

		wait();
	}
}

void test_fir4dlms::compare() {
